...
class TestSICRephraseClient:
    ...
    def test_init_with_environment_variable(self):
        """Test initialisation using environment variable."""
        env_path = "/env/path/rephrased.csv"

        with patch.dict(os.environ, {"SIC_REPHRASE_DATA_PATH": env_path}), patch(
            "pandas.read_csv"
        ) as mock_read_csv:
            mock_df = mock_read_csv.return_value
            mock_df.columns = ["sic_code", "reviewed_description"]
            ...
            mock_read_csv.assert_called_once_with(env_path, dtype={"sic_code": str})
    ...
    def test_init_with_local_fallback(self):
        """Test initialisation using local fallback path."""
        with patch.dict(os.environ, {}, clear=True), patch(
            "importlib.util.find_spec"
        ) as mock_find_spec:
            mock_find_spec.return_value = None  # Simulate module not found
            ...
            with patch("pandas.read_csv") as mock_read_csv:
                ...
                # Should call pandas.read_csv with local path
                mock_read_csv.assert_called_once()
                call_args = mock_read_csv.call_args[0][0]
                assert "data/example_rephrased_sic_data.csv" in call_args
    ...
    def test_process_classification_response(self):
        ...
        with patch("importlib.util.find_spec") as mock_find_spec:
            ...
            with patch("pandas.read_csv") as mock_read_csv:
                ...
                processed_response = client.process_classification_response(
                    response_data
                )
