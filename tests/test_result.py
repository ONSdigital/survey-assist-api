...
class TestGenericResultEndpoint:  # pylint: disable=attribute-defined-outside-init
    """Test class for the generic result endpoint."""
    ...
    @patch("api.routes.v1.result.store_result")
    def test_store_generic_survey_result_success(self, mock_store_result):
        """Test successful storage of a generic survey result."""
        mock_store_result.return_value = None
                            ...
                            "response": [
                                {
                                    ...
                                    "candidates": [
                                        {
                                            "code": "01110",
                                            "descriptive": "Growing of cereals",
                                            "likelihood": 0.8,
                                        }
                                    ],
                                    "reasoning": "SIC classification reasoning",
                                },
                                {
                                    "type": "soc",
                                    "classified": True,
                                    ...
                                    "reasoning": "SOC classification reasoning",
                                },
                            ],
                        }
                    ],
        ...
        response = client.post("/v1/survey-assist/result/v2", json=result_data)
        assert response.status_code == status.HTTP_200_OK

        data = response.json()
        assert data["message"] == "Generic result stored successfully"
        assert data["result_id"] is not None
        assert "test-survey-123/test.userSA187/" in data["result_id"]
        assert data["result_id"].endswith("_v2.json")

    @patch("api.routes.v1.result.store_result")
    def test_store_generic_survey_result_error(self, mock_store_result):
        """Test error handling when storing a generic survey result fails."""
        mock_store_result.side_effect = Exception("Storage error")
    ...
    @patch("api.routes.v1.result.get_result")
    def test_get_generic_survey_result_success(self, mock_get_result):
        """Test successful retrieval of a generic survey result."""
        mock_result_data = {
            "survey_id": "test-survey-123",
            "case_id": "test-case-456",
            "user": "test.userSA187",
        ...
        response = client.get("/v1/survey-assist/result/v2?result_id=test-id")
        assert response.status_code == status.HTTP_200_OK
        ...
        assert len(data["responses"][0]["survey_assist_interactions"]) == 1

        interaction = data["responses"][0]["survey_assist_interactions"][0]
        assert interaction["flavour"] == "sic"
        assert len(interaction["response"]) == 1

        sic_result = interaction["response"][0]
        assert sic_result["type"] == "sic"
        assert sic_result["classified"] is True
        assert sic_result["code"] == "43210"

    @patch("api.routes.v1.result.get_result")
    def test_get_generic_survey_result_not_found(self, mock_get_result):
        """Test error handling when retrieving a non-existent generic survey result."""
        mock_get_result.side_effect = FileNotFoundError("Result not found")

        response = client.get("/v1/survey-assist/result/v2?result_id=non-existent")
        assert response.status_code == status.HTTP_404_NOT_FOUND
        assert "Result not found" in response.json()["detail"]

    @patch("api.routes.v1.result.get_result")
    def test_get_generic_survey_result_error(self, mock_get_result):
        """Test error handling when retrieving a generic survey result fails."""
        mock_get_result.side_effect = Exception("Retrieval error")
